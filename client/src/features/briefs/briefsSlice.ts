...
export const analyzeBrief = createAsyncThunk(
  ...
  async (id: string, { rejectWithValue }) => {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        return rejectWithValue(errorData.message || 'Failed to analyze brief');
      }
      
      const data = await response.json();
      return { id, analysis: data.data };
    } catch (error: any) {
      return rejectWithValue(error.message || 'Failed to analyze brief');
    }
  }
);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(analyzeBrief.fulfilled, (state, action: PayloadAction<{ id: string; analysis: BriefAnalysis }>) => {
        ...
        // Update current brief if it's the one that was analyzed
        if (state.currentBrief?.id === action.payload.id) {
...
export const selectBriefsTotalCount = (state: RootState) => state.briefs.totalCount;
