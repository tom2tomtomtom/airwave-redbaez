...
class LLMService {
   ...
   */
  private formatBriefForMotivationsPrompt(briefData: BriefData): string {
    return `
  ...
  private parseMotivationsFromResponse($1: unknown): Motivation[] {
    try {
      ...
      return motivationsData.map((item: any, index: number) => ({
   ...
   */
  private extractSection(text: string, sectionName: string, maxLen: number = 200): string | null {
  ...
  async analyzeBriefText(extractedText: string, partialBrief: Partial<BriefData>): Promise<Partial<BriefData>> {
    try {
      ...
      if (!fallbackResult.projectName) fallbackResult.projectName = 'Marketing Campaign';
      if (!fallbackResult.productDescription) fallbackResult.productDescription = 'Product description based on uploaded document';
      if (!fallbackResult.targetAudience) fallbackResult.targetAudience = 'General consumers';
      if (!fallbackResult.campaignObjectives) fallbackResult.campaignObjectives = 'Increase brand awareness and drive engagement';
   ...
   */
  private formatCopyGenerationPrompt(briefData: BriefData, motivations: Motivation[], request: CopyGenerationRequest): string {
    // Format motivations for inclusion in prompt
    const motivationsText = motivations.map(m => 
   ...
   */
  private parseCopyVariationsFromResponse(response: any, request: CopyGenerationRequest): CopyVariation[] {
    try {
      // Extract the content from the response
      let content = response.choices?.[0]?.message?.content;
      ...
      return variationsData.map((item: any, index: number) => ({
