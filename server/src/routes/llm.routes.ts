...
router.post('/parse-brief', checkAuth, async (req, res) => {
  try {
    ...
    const missingFields = [];
    if (!briefData.clientName) missingFields.push('clientName');
    if (!briefData.projectName) missingFields.push('projectName');
    if (!briefData.productDescription) missingFields.push('productDescription');
    if (!briefData.targetAudience) missingFields.push('targetAudience');
    if (!briefData.campaignObjectives) missingFields.push('campaignObjectives');
...
router.post('/process-brief-document', checkAuth, upload.single('brief'), async (req, res) => {
  try {
    ...
    try {
      ...
      const completeBriefData = await llmService.analyzeBriefText(extractedText, {
        ...briefData,
