...
async function main() {
    try {
        ...
        let fixedContent = content.replace(/\/\/ This is just a JSDoc comment for the above method, the actual implementation is earlier in the file\s+try\s+{/, `/**
  ...
  async getAssetsByClientSlug(slug: string, options: any = {}): Promise<{assets: Asset[], total: number}> {
    try {`);
        ...
        fs_1.default.writeFileSync(servicePath, fixedContent, 'utf8');
