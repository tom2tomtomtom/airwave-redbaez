...
async function main() {
    ...
    try {
        ...
        const { data: assets, error: assetError, count } = await supabaseClient_1.supabase
            .from('assets')
            .select('*', { count: 'exact' })
        ...
        const { data: sampleAsset, error: sampleError } = await supabaseClient_1.supabase
            .from('assets')
            .select('*')
            .limit(1)
            .single();
        ...
        const { data: routes, error: routesError } = await supabaseClient_1.supabase
            .from('routes')
            .select('*')
