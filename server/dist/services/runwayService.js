...
class RunwayService {
    constructor(apiKey = RUNWAY_API_KEY, baseUrl = RUNWAY_API_URL) {
        this.apiKey = apiKey;
    ...
    calculateAspectRatioString(width, height) {
    ...
    async pollForTaskResults(taskId, maxAttempts = 30, interval = 5000) {
        let attempts = 0;
        ...
        while (attempts < maxAttempts) {
            try {
                ...
                const response = await axios_1.default.get(`${this.baseUrl}/${taskId}`, // Use baseUrl with taskId appended
                {
                    headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json',
    ...
    calculateAspectRatioFromDimensions(width, height) {
    ...
    async pollForResults(jobId, maxAttempts = 30, interval = 1000) {
        let attempts = 0;
        while (attempts < maxAttempts) {
            try {
                console.log(`Polling for job ${jobId}, attempt ${attempts + 1}/${maxAttempts}`);
                // Check the generation status
                const response = await axios_1.default.get(`https://api.runwayml.com/v1/generations/${jobId}`, {
                    headers: {
                        'Authorization': `Bearer ${this.apiKey}`,
                        'Accept': 'application/json',
                        'runway-api-version': '0' // Required header for Runway API
