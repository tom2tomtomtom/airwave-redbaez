...
class BriefService {
     ...
     */
    async queueBriefForAnalysis(briefId) {
        try {
     ...
     * @param briefId ID of the brief to analyze
     */
    async analyzeBrief(briefId) {
        try {
            // Get the brief from the database
            const { data: brief, error: fetchError } = await supabaseClient_1.supabase
                .from('briefs')
                .select('*')
                .eq('id', briefId)
                .single();
      ...
      `;
            ...
            try {
                const { data: brief } = await supabaseClient_1.supabase
                    .from('briefs')
                    .select('meta')
                    .eq('id', briefId)
                    .single();
            ...
            if (filters.searchTerm) {
                ...
                if (searchTerm.length < 3 || !searchTerm.includes(' ')) {
                    ...
                    const formattedSearchTerm = searchTerm
                        ...
                        .map(word => word + ':*')
                        .join(' & ');
                    query = query.or(`title.wfts.${formattedSearchTerm},content.wfts.${formattedSearchTerm}`);
                }
            }
            // Apply sorting
     ...
     */
    async updateBrief(id, userId, updates) {
        try {
            // Get the current brief for security check
            const { data: existingBrief, error: fetchError } = await supabaseClient_1.supabase
                .from('briefs')
                .select('*')
                .eq('id', id)
                .single();
            ...
            if (updates.tags || updates.insights || updates.analysis) {
                ...
                if (updates.tags)
                    updatedMeta.tags = updates.tags;
                if (updates.insights)
                    updatedMeta.insights = updates.insights;
                if (updates.analysis)
                    updatedMeta.analysis = updates.analysis;
     ...
     */
    async generateContent(briefId, userId, options) {
        try {
            // Get the brief from the database
            ...
            const { targetAudience, keyMessages, toneOfVoice, campaignObjectives } = brief.analysis;
            // Prepare the prompt for OpenAI
            const prompt = `
        ...
        LENGTH GUIDANCE: ${lengthGuidance[targetLength]}
      ...
      `;
            ...
            try {
                const { data: briefData } = await supabaseClient_1.supabase
                    .from('briefs')
                    .select('meta')
                    .eq('id', briefId)
                    .single();
