...
class LLMService {
    ...
    async processBrief(briefData) {
        try {
            ...
            const { data, error } = await supabaseClient_1.supabase.functions.invoke('process-brief', {
                body: briefData
            });
            if (error) {
            ...
            return data.motivations.map((motivation, index) => ({
    ...
    async regenerateMotivations(briefData, feedback) {
        try {
            ...
            const { data, error } = await supabaseClient_1.supabase.functions.invoke('regenerate-motivations', {
    ...
    async generateCopy(request, briefData, motivations) {
        try {
            ...
            const { data, error } = await supabaseClient_1.supabase.functions.invoke('generate-copy', {
                body: {
                    brief: briefData,
            ...
            return data.copyVariations.map((variation, index) => ({
