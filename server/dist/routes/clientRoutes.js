...
const addClientSlugs_1 = require("../migrations/addClientSlugs");
...
const clientValidationRules = [
    (0, express_validator_1.body)('name').notEmpty().withMessage('Client name is required').trim(),
    (0, express_validator_1.body)('logo_url').optional({ nullable: true }).isURL().withMessage('Logo URL must be a valid URL'),
    (0, express_validator_1.body)('primary_color').optional({ nullable: true }).isHexColor().withMessage('Primary colour must be a valid hex colour'),
    (0, express_validator_1.body)('secondary_color').optional({ nullable: true }).isHexColor().withMessage('Secondary colour must be a valid hex colour'),
    (0, express_validator_1.body)('description').optional({ nullable: true }).trim(),
...
router.post('/run-migration', async (req, res) => {
    try {
        ...
        await (0, addClientSlugs_1.runClientSlugMigration)();
...
// Create a new client
router.post('/', clientValidationRules, async (req, res) => {
    try {
        console.log('Client creation payload:', req.body);
        const mappedData = {};
        // Direct mappings
        if (req.body.name)
            mappedData.name = req.body.name;
        if (req.body.description)
            mappedData.description = req.body.description;
        ...
        if (req.body.isActive !== undefined)
            mappedData.is_active = req.body.isActive;
        // Handle British spelling to American spelling conversion
        if (req.body.brandColour)
            mappedData.primary_color = req.body.brandColour;
        if (req.body.secondaryColour)
            mappedData.secondary_color = req.body.secondaryColour;
        // Also handle if someone is using American spelling on the frontend
        if (req.body.brandColor)
            mappedData.primary_color = req.body.brandColor;
        if (req.body.secondaryColor)
            mappedData.secondary_color = req.body.secondaryColor;
        ...
        const generatedSlug = await (0, addClientSlugs_1.generateClientSlug)(clientName);
        ...
        const { data: existingClient } = await supabaseClient_1.supabase
            .from('clients')
            .select('id')
 ...
 */
router.post('/create-asset-lookup', async (req, res) => {
    try {
...
// Update a client
router.put('/:id', clientValidationRules, async (req, res) => {
    try {
...
async function getClientRelatedCounts(clientId) {
    // Get asset count
