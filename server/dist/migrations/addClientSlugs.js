...
exports.generateClientSlug = generateClientSlug;
exports.addClientSlugColumn = addClientSlugColumn;
exports.migrateClientsToSlugs = migrateClientsToSlugs;
exports.runClientSlugMigration = runClientSlugMigration;
exports.getClientBySlug = getClientBySlug;
const supabaseClient_1 = require("../db/supabaseClient");
/**
 * Generates a URL-friendly slug from a client name
 */
function generateClientSlug(name) {
    return name
        .toLowerCase()
 ...
 */
async function ensureUniqueSlug(slug, clientId) {
    ...
    while (true) {
        const { data: check } = await supabaseClient_1.supabase
            .from('clients')
            .select('id')
 ...
 */
async function addClientSlugColumn() {
    console.log('Checking if client_slug column exists...');
    // Check if column exists
    const { data: columnExists, error: checkError } = await supabaseClient_1.supabase
        .rpc('column_exists', {
        table_name: 'clients',
    ...
    if (checkError) {
        ...
        try {
            ...
            await supabaseClient_1.supabase.rpc('add_client_slug_column');
 ...
 */
async function migrateClientsToSlugs() {
    ...
    try {
        await supabaseClient_1.supabase.rpc('create_client_slug_index');
 ...
 */
async function runClientSlugMigration() {
